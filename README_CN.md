...
>
> 当前仓库的“科研生图表智能体”精简环境请优先阅读
> [`WINDOWS_DEPLOY_RESEARCH_AGENT.md`](WINDOWS_DEPLOY_RESEARCH_AGENT.md)。
> `requirements-win-base.txt` 已包含通用与 Paper/Figure 依赖，不要重复安装
> `requirements-paper.txt`，也不要在 Windows 安装 `requirements-cu12.txt`。
