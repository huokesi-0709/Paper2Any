...
class TechnicalRouteColorizeSvgAgent(BaseAgent):
    ...
    def get_task_prompt_params(self, pre_tool_results: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "color_template_svg": pre_tool_results.get("color_template_svg", ""),
        }
...
"""
        ...
        def validate_colors_applied(content: str, parsed_result: Dict[str, Any]) -> Tuple[bool, str]:
            """验证配色是否正确应用"""
            svg_code = parsed_result.get("svg_code", "")
            if not svg_code:
            ...
            # 检查是否还有黑白灰色（应该被替换为配色）
            if 'fill="black"' in svg_code or 'fill="#000"' in svg_code or 'fill="#000000"' in svg_code:
                return False, "检测到黑色填充，请使用配色方案中的颜色替换所有黑色"
        ...
        return [validate_svg_structure, validate_chinese_font, validate_colors_applied, validate_language_requirement, validate_svg_renderable]
