...
from dataflow_agent.utils.request_options import apply_chat_model_options, chat_model_options
...
class AIProviderStrategy(ABC):
    ...
    def build_chat_request(
        ...
        **kwargs
        ...
        apply_chat_model_options(payload, model)
        return url, payload
...
class QwenImageProvider(AIProviderStrategy):
    """Alibaba Cloud Model Studio native Qwen-Image API."""
    ...
    @staticmethod
    def _generation_url(api_url: str) -> str:
        base = api_url.rstrip("/")
    ...
    @classmethod
    def _output_size(cls, aspect_ratio: str, resolution: str) -> str:
        try:
        ...
        long_edge = cls._MAX_EDGE if str(resolution).upper() in {"2K", "4K"} else 1024
        if width_ratio >= height_ratio:
            width = long_edge
            height = max(8, round((long_edge * height_ratio / width_ratio) / 8) * 8)
        else:
            height = long_edge
            width = max(8, round((long_edge * width_ratio / height_ratio) / 8) * 8)

        pixels = width * height
        if pixels < cls._MIN_PIXELS:
            scale = (cls._MIN_PIXELS / pixels) ** 0.5
            width = min(cls._MAX_EDGE, max(8, round((width * scale) / 8) * 8))
            height = min(cls._MAX_EDGE, max(8, round((height * scale) / 8) * 8))
        return f"{width}*{height}"

    @staticmethod
    def _parameters(kwargs: Dict[str, Any], *, edit: bool) -> Dict[str, Any]:
        parameters: Dict[str, Any] = {
            "prompt_extend": kwargs.get("prompt_extend", True),
            "prompt_extend_mode": "direct" if edit else kwargs.get("prompt_extend_mode", "direct"),
            "n": int(kwargs.get("n", 1)),
            "watermark": bool(kwargs.get("watermark", False)),
        }
        parameters["size"] = QwenImageProvider._output_size(
    ...
    def build_generation_request(
        self, api_url: str, model: str, prompt: str, **kwargs
    ) -> Tuple[str, Dict[str, Any], bool]:
        payload = {
            "model": model,
            "input": {
            ...
            "parameters": self._parameters(kwargs, edit=False),
        }
        return self._generation_url(api_url), payload, False
    ...
    def build_edit_request(
        ...
        **kwargs,
        ...
        image_fmt = kwargs.get("image_fmt", "png")
        image_data = f"data:image/{image_fmt};base64,{image_b64}"
        payload = {
            "model": model,
            "input": {
                "messages": [
                    {
                        "role": "user",
                        "content": [{"image": image_data}, {"text": prompt}],
                    }
                ]
            },
            "parameters": self._parameters(kwargs, edit=True),
        }
        return self._generation_url(api_url), payload, False
    ...
    def parse_generation_response(self, data: Dict[str, Any]) -> str:
        ...
        if data.get("code") or data.get("message"):
            raise RuntimeError(
                f"Qwen-Image API error {data.get('code', '')}: {data.get('message', '')}".strip()
            )
        raise RuntimeError(f"Failed to parse Qwen-Image response: {str(data)[:300]}")
...
class LLMClient:
    ...
    def _generate_openai_compatible(
        ...
        response = self._client.chat.completions.create(
            ...
            messages=messages,
            stream=False,
            **chat_model_options(model_name, temperature=temperature),
        )
        final_text = ""
