...
export default function Paper2DrawioPage({
  ...
  const modeButtonActive = 'bg-gradient-to-r from-neon-cyan to-neon-purple text-lab-primary shadow-[0_0_30px_rgba(0,229,255,0.4),0_0_60px_rgba(124,58,237,0.3)] border border-neon-cyan/50 scale-105';
  const modeButtonIdle = 'bg-surface-base/40 text-slate-600 hover:bg-neon-cyan/10 hover:text-neon-cyan hover:border-neon-cyan/40 hover:scale-105 border border-border-medium';
  ...
  return (
    <div className="page-shell overflow-y-auto">
      ...
      <div className="page-container pt-8 pb-8">
        ...
        <div className="workflow-studio mt-6" style={{ minHeight: '720px' }}>
          {/* 左侧：输入区域 */}
          <div className="workflow-configuration flex flex-col gap-4" style={{ animationDelay: '40ms' }}>
            {showModePanel && (
              <div className={panelClass}>
                ...
                <div className="space-y-2 text-xs text-slate-600">
                  <div className="relative group rounded-xl border border-border-medium bg-surface-base/40 p-3 hover:border-neon-cyan/40 hover:bg-surface-base/60 transition-all cursor-pointer">
                    <p className="text-lab-primary font-semibold mb-1">Demo · AI 驱动</p>
                    ...
                    <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[320px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 pointer-events-none" style={{ zIndex: 9999 }}>
                      <div className="rounded-lg border border-neon-cyan/60 bg-white/98 backdrop-blur-xl shadow-2xl overflow-hidden">
                        <div className="px-2.5 py-1.5 border-b border-border-medium flex items-center justify-between">
                          ...
                          <span className="text-[9px] text-slate-600">悬停查看</span>
                        </div>
                        <div className="p-1.5">
                          <img
                  ...
                  <div className="relative group rounded-xl border border-border-medium bg-surface-base/40 p-3 hover:border-neon-cyan/40 hover:bg-surface-base/60 transition-all cursor-pointer">
                    <p className="text-lab-primary font-semibold mb-1">Demo · 科研绘图</p>
            ...
            <details className={panelClass}>
              <summary className="cursor-pointer text-sm font-semibold text-slate-700">{t('apiConfig')}</summary>
              <h3 className="text-sm font-display font-bold text-lab-primary mb-3 flex items-center gap-2">
                <Wand2 className="text-neon-cyan" size={18} />
                {t('apiConfig')}
              </h3>
              <div className="space-y-2">
                {userApiConfigRequired ? (
                  <>
                    <div className="flex items-center justify-between">
                      <label className="block text-xs text-slate-600">{t('apiUrl')}</label>
                        <a
                    ...
                    {generationMode === 'paper2drawio' ? (
                      ...
                      >
                        {API_URL_OPTIONS.map((url: string) => (
                          <option key={url} value={url} className="bg-white">{url}</option>
                        ))}
                      </select>
                    ) : (
                      <input
                ...
                {!userApiConfigRequired && (
                  <p className="text-[11px] leading-5 text-emerald-800">Free 模式下由后端统一选择 DrawIO 生成模型。</p>
                )}
                {userApiConfigRequired && modelOptions.length > 1 && (
                  <label className="flex items-start gap-2 rounded-xl border border-border-medium bg-surface-base/40 px-3 py-2 text-xs text-slate-600">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span>
                      <span className="block text-slate-800">{t('modelRace')}</span>
                      <span className="text-slate-600">{t('modelRaceHint')}</span>
                    </span>
                  </label>
                )}
              </div>
              ...
              {uploadMode === 'text' ? (
                ...
                <div className="border border-dashed border-border-medium rounded-xl p-6 text-center hover:border-neon-cyan/40 transition-colors">
                  ...
                  <label htmlFor="pdf-upload" className="cursor-pointer">
                    ...
                    <p className="text-sm text-lab-primary font-medium mb-1">
            ...
            {generationMode === 'paper2drawio' && (
              <div className={panelClass}>
                ...
                <div className="space-y-2">
                  ...
                  {!userApiConfigRequired && (
                    <p className="text-[11px] leading-5 text-emerald-800">Free 模式下由后端统一选择图表生成模型。</p>
                  )}
                  <select
                    value={p2dLanguage}
                    ...
                    className={inputClass}
                  >
                    <option value="zh" className="bg-white">{t('modelParams.language.zh')}</option>
                    <option value="en" className="bg-white">{t('modelParams.language.en')}</option>
                  </select>
                  <select
                    value={p2dStyle}
                    ...
                    className={inputClass}
                  >
                    <option value="cartoon" className="bg-white">{t('modelParams.style.cartoon')}</option>
                    <option value="realistic" className="bg-white">{t('modelParams.style.realistic')}</option>
                  </select>
                  <select
                    ...
                    className={inputClass}
                  >
                    <option value="easy" className="bg-white">{t('modelParams.complexity.easy')}</option>
                    <option value="mid" className="bg-white">{t('modelParams.complexity.mid')}</option>
                    <option value="hard" className="bg-white">{t('modelParams.complexity.hard')}</option>
                  </select>
                </div>
              </div>
            )}
            ...
            {generationMode === 'ai' && (
              ...
              <select
                ...
                className={inputClass}
              >
                <option value="auto" className="bg-white">{t('auto')}</option>
                <option value="flowchart" className="bg-white">{t('flowchart')}</option>
                <option value="architecture" className="bg-white">{t('architecture')}</option>
                <option value="sequence" className="bg-white">{t('sequence')}</option>
                <option value="mindmap" className="bg-white">{t('mindmap')}</option>
                <option value="er" className="bg-white">{t('er')}</option>
              </select>
              </div>
            )}
            ...
            {generationMode === 'ai' && (
              <div className={panelClass}>
                ...
                <div className="space-y-2">
                  <div className="text-xs text-slate-600">{t('diagramLanguage')}</div>
                  <select
                        ...
                        <div
                          className={`max-w-[85%] px-4 py-2.5 text-sm rounded-[20px] ${
                            msg.role === 'user'
                              ...
                              : 'bg-surface-base text-slate-800 rounded-bl-sm shadow-[0_0_12px_rgba(0,0,0,0.3)]'
                          }`}
                        >
                  ...
                  <div className="p-3 border-t border-border-medium">
                    <div className="relative flex items-center">
                      ...
                      <button
                        ...
                        className="absolute right-1.5 p-1.5 rounded-full bg-neon-cyan text-lab-primary hover:bg-neon-purple disabled:opacity-0 disabled:pointer-events-none transition-all duration-200 shadow-[0_0_12px_rgba(0,229,255,0.4)]"
                      >
          ...
          {/* 右侧：预览区域 */}
          <div className="workflow-output bento-card flex flex-col p-4 md:p-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center rounded-xl bg-surface-base/40 border border-border-medium px-3 py-2">
                    <input
                      ...
                      className="w-24 bg-transparent text-xs text-lab-primary placeholder-slate-500 outline-none"
                      placeholder="diagram"
                    />
                    <span className="ml-2 text-xs text-slate-600">.{exportFormat}</span>
                  </div>
                  <button
            ...
            {generationMode === 'paper2drawio' && showP2dPng && p2dPngUrl && (
              <div className="bento-card scan-line mt-4 p-3">
                <div className="text-xs text-slate-600 mb-2">科研绘图 PNG 预览</div>
                <div className="rounded-xl overflow-hidden border border-border-medium bg-slate-50">
                  <img src={p2dPngUrl} alt="paper2drawio png preview" className="w-full h-auto object-contain" />
                </div>
              </div>
            )}
            <div className="mt-4 flex-1 bg-slate-50 rounded-xl border border-border-medium min-h-[420px] lg:min-h-[720px] overflow-hidden relative">
              <iframe
                key={drawioFrameKey}
                ref={iframeRef}
              ...
              {!xmlContent && (
                <div className="absolute inset-0 empty-state">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">{t('previewPlaceholder')}</p>
                  </div>
                </div>
              )}
              {xmlContent && !drawioReady && (
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-50/85 backdrop-blur-sm text-center">
                  <div className="space-y-3">
                    <div className="mx-auto h-8 w-8 rounded-full border-2 border-white/20 border-t-neon-cyan animate-spin" />
                    <p className="text-sm text-slate-600">正在初始化 DrawIO 预览...</p>
