...
async def run_paper2figure_wf_api_local(req: Paper2FigureRequest, result_path: Path | None = None) -> Paper2FigureResponse:
    ...
    if not _has_valid_paper2figure_output(
        ...
        # Image-only workflow keeps the upstream provider error in agent_results.
        # Prefer that detail over a generic missing-output message.
        agent_results = (
            final_state.get("agent_results", {})
            if isinstance(final_state, dict)
            else getattr(final_state, "agent_results", {})
        ) or {}
        image_error = agent_results.get("gen_img_error", {}) if isinstance(agent_results, dict) else {}
        error = str(image_error.get("msg") or "生成失败：后端未产出有效文件，请检查后端日志。")
        log.error(
