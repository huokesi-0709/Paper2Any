...
from dataflow_agent.utils.request_options import apply_chat_model_options
...
class FigureMindService:
    ...
    async def verify_llm_connection(self, req: VerifyLlmRequest) -> VerifyLlmResponse:
        ...
        apply_chat_model_options(payload, req.model)
    ...
    async def generate_paper2figure_json(
        ...
        if graph_type == "model_arch" and not has_preview_image:
            return {
                "success": False,
                "error": (
                    (p2f_resp.error or "").strip()
                    or "生成失败：后端未产出有效预览图片（PNG/JPG/WebP），请检查生成日志或模型输出。"
                ),
