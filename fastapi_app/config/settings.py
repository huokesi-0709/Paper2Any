...
def _apply_simple_mode(settings_obj: AppSettings) -> AppSettings:
    ...
    reasoning_effort = _first_non_empty(settings_obj.SIMPLE_REASONING_EFFORT)

    # Request helpers live below the settings layer; publish the resolved value
    # so every retained OpenAI-compatible caller receives the same setting.
    if reasoning_effort:
        os.environ["SIMPLE_REASONING_EFFORT"] = reasoning_effort
    ...
    settings_obj.IMAGE_PLAYGROUND_DEFAULT_IMAGE_MODEL = image_model
